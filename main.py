...
async def chek_captcha(callback: CallbackQuery, state: FSMContext):
    ...
    users_link = f'<a href="tg://user?id={callback.from_user.id}">{callback.from_user.full_name}</a>'
    Casinoo_link = f'<a href="https://t.me/LuciFerCasino1"><b>{NAME_CASINO}</b></a>'
    rules_link = '<a href="https://telegra.ph/Pravila-i-soglashenie-LuciFerCasino-05-27-2">Пользовательское соглашение</a>'
    ...
    if keys == word:
        ...
        await callback.message.answer(
            f'<b>👋 Добро пожаловать, {users_link}!</b>\n\n'
            f'🎰 Добро пожаловать в {Casinoo_link} - место, где рождаются победители!\n\n'
            f'📜 <b>Перед началом игры важно ознакомиться с {rules_link}.</b>\n'
            f'Это займёт всего пару минут, но поможет избежать недоразумений.\n\n'
            f'❗ <i>Нажимая кнопку "Я согласен✅", вы подтверждаете, что:\n'
            f'- Вам исполнилось 18 лет\n'
            f'- Вы ознакомились с правилами\n'
            f'- Принимаете условия игры</i>',
            reply_markup=ikb_accept_rules(),
            parse_mode='HTML',
            disable_web_page_preview=True
        )
        await state.set_state(AcceptRules.waiting_accept)
    else:
        await callback.answer('⚠️ Вы не прошли проверку!', show_alert=True)
        await callback.message.edit_text(
            text=f'👋 Привет, {callback.from_user.first_name}!\n\n'
                 '🤖 Чтобы доказать, что ты не робот, нажми на кнопку 👇\n\n'
                 f'<b>👉 {word_new} 👈</b>',
            reply_markup=await captcha_keybord(word_new),
            parse_mode='HTML'
        )

@dp.callback_query(F.data == 'accept_rules', AcceptRules.waiting_accept)
async def accept_rules(callback: CallbackQuery, state: FSMContext):
    users_link = f'<a href="tg://user?id={callback.from_user.id}">{callback.from_user.full_name}</a>'
    Casinoo_link = f'<a href="https://t.me/LuciFerCasino1"><b>{NAME_CASINO}</b></a>'
    
    await callback.message.edit_text(
        f'<b>👋 Добро пожаловать {users_link} в {Casinoo_link} 🎲</b>\n\n'
        '🎉 Теперь вы полноправный участник нашего казино!\n\n'
